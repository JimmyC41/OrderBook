...
bool InputHandler::TryParseOrderInfo(const std::string_view& str, Information& event) const {
    auto value = str.at(0);
    auto values = Split(str, ' ');
...
Price InputHandler::ParsePrice(const std::string_view& str) const {
    if (str.empty()) throw std::logic_error("Price N/A.");
    return ToNumber(str);
}

Quantity InputHandler::ParseQuantity(const std::string_view& str) const {
    if (str.empty()) throw std::logic_error("Quantity N/A.");
    return ToNumber(str);
}

std::tuple<Informations, Result> InputHandler::GetOrderInformations(const std::filesystem::path& path) const {
    Informations events;
    ...
    while (std::getline(file, line)) {
        ...
        if (isEvent) {
            ...
            if (!file.eof()) throw std::logic_error("Result should only be specified at the end.");
